...
class GraspOnlyClient:
    def __init__(
        ...
        *,
        ...
        self.num_visualize_grasps = num_visualize_grasps
        self.block_after_visualization = block_after_visualization
        self.auto_start_meshcat = auto_start_meshcat
        self.open_browser = open_browser
        self.proc: Optional[subprocess.Popen] = None
        self.meshcat_proc: Optional[subprocess.Popen] = None
        self.registry: Optional[ProfileRegistry] = None

    def _maybe_start_meshcat_server(self) -> None:
        ...
        if not self.auto_start_meshcat:
            rospy.logwarn(
                "可视化已启用，但未检测到 MeshCat server。"
                f"请先在 {self.grasp_env} 环境中运行 `meshcat-server`，然后打开 {MESHCAT_URL}"
            )
            return

        conda_sh = "/home/songfei/miniconda3/etc/profile.d/conda.sh"
        launch_cmd = (
            f"source {conda_sh} && conda activate {self.grasp_env} && meshcat-server"
        )
        rospy.loginfo("启动 MeshCat server 用于浏览器可视化...")
        self.meshcat_proc = subprocess.Popen(
            ["bash", "-lc", launch_cmd],
    ...
    def start(self) -> None:
        ...
        self._maybe_start_meshcat_server()
        ...
        profile = self.registry.get(
            ...
            override={
                ...
                "num_visualize_grasps": self.num_visualize_grasps,
                "block_after_visualization": self.block_after_visualization,
    ...
    def stop(self) -> None:
        ...
        if self.meshcat_proc is not None:
            try:
                self.meshcat_proc.terminate()
                self.meshcat_proc.wait(timeout=2.0)
            except Exception:
                try:
                    self.meshcat_proc.kill()
                except Exception:
                    pass
            finally:
                self.meshcat_proc = None
...
def main() -> None:
    rospy.init_node("grasp_view_vision")
    rospy.loginfo("========== Grasp View Vision ==========")
    ...
    enable_visualization = bool(rospy.get_param("~grasp/enable_visualization", True))
    num_visualize_grasps = int(rospy.get_param("~grasp/num_visualize_grasps", 10))
    block_after_visualization = bool(
        rospy.get_param("~grasp/block_after_visualization", False)
    )
    auto_start_meshcat = bool(rospy.get_param("~grasp/auto_start_meshcat", True))
    open_browser = bool(rospy.get_param("~grasp/open_browser", True))
