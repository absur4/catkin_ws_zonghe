...
# ============================================================
# 导航中间点配置（90cm 门口避障）
# A 点：kitchen / dining_table（同一坐标）
# 目标点：cabinet / dishwasher / kitchen_surface
# A -> 目标：经过 mid_1_1, mid_1_2
# 目标 -> A：经过 mid_2_1, mid_2_2
# ============================================================
A_LOCATIONS = {'kitchen', 'dining_table'}
TARGET_LOCATIONS = {'cabinet', 'dishwasher', 'kitchen_surface'}
MIDPOINTS_FORWARD = ['mid_1_1', 'mid_1_2']
MIDPOINTS_RETURN = ['mid_2_1', 'mid_2_2']
CURRENT_LOCATION = None


def _set_current_location(name):
    global CURRENT_LOCATION
    CURRENT_LOCATION = name


def _plan_route(current, target):
    if current in A_LOCATIONS and target in TARGET_LOCATIONS:
        return MIDPOINTS_FORWARD + [target]
    if current in TARGET_LOCATIONS and target in A_LOCATIONS:
        return MIDPOINTS_RETURN + [target]
    return [target]


def _send_nav_goal(nav_client, location_name, log_prefix):
    if not nav_client.wait_for_server(timeout=rospy.Duration(10.0)):
        rospy.logerr(f"{log_prefix} 导航动作服务器未响应")
        return False

    goal = NavigateToLocationGoal()
    goal.target_location = location_name

    rospy.loginfo(f"{log_prefix} 发送导航目标: {location_name}")
    nav_client.send_goal(goal)

    finished = nav_client.wait_for_result(timeout=rospy.Duration(120.0))
    if not finished:
        nav_client.cancel_goal()
        rospy.logerr(f"{log_prefix} 导航到 {location_name} 超时")
        return False

    result = nav_client.get_result()
    if result and result.success:
        _set_current_location(location_name)
        rospy.loginfo(f"{log_prefix} ✓ 成功到达 {location_name}")
        return True

    rospy.logwarn(f"{log_prefix} ✗ 导航到 {location_name} 失败")
    return False


def _navigate_with_midpoints(nav_client, target_name, log_prefix):
    route = _plan_route(CURRENT_LOCATION, target_name)
    if len(route) > 1:
        rospy.loginfo(f"{log_prefix} 规划中间点路径: {route}")
    for waypoint in route:
        if not _send_nav_goal(nav_client, waypoint, log_prefix):
            return False
    return True
...
class NavigateToKitchen(smach.State):
    ...
    def execute(self, userdata):
        rospy.loginfo("========== 导航到厨房 ==========")
        if _navigate_with_midpoints(self.nav_client, "kitchen", "[NavigateToKitchen]"):
            return 'arrived'
...
class NavigateBackToKitchen(smach.State):
    ...
    def execute(self, userdata):
        ...
        if _navigate_with_midpoints(self.nav_client, "kitchen", "[NavigateBackToKitchen]"):
...
class NavigateToDest(smach.State):
    ...
    def execute(self, userdata):
        ...
        if _navigate_with_midpoints(self.nav_client, destination, "[NavigateToDest]"):
            return 'arrived'
        return 'navigation_failed'
...
class ServeBreakfast(smach.State):
    ...
    def _navigate_to(self, location_name):
        ...
        return _navigate_with_midpoints(self.nav_client, location_name, "[ServeBreakfast]")
