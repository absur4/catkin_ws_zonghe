...
class JointMoveitCtrlServer:
    def __init__(self):
        ...
        self.stop_arm_srv = rospy.Service('joint_moveit_stop_arm', Trigger, self.handle_joint_moveit_stop_arm)
    ...
    def handle_joint_moveit_ctrl_endpose(self, request):
        ...
        try:
            if self.arm_move_group:
                ...
                self.arm_move_group.set_max_acceleration_scaling_factor(max_acceleration)
                rospy.loginfo(f"max_velocity: {max_velocity} max_acceleration: {max_acceleration}")
                ok = bool(self.arm_move_group.go(wait=True))
                self.arm_move_group.stop()
                self.arm_move_group.clear_pose_targets()
                if ok:
                    rospy.loginfo("Endpose movement executed successfully.")
                    return JointMoveitCtrlResponse(status=True, error_code=0)
                rospy.logwarn("Endpose movement failed.")
        ...
        return JointMoveitCtrlResponse(status=False, error_code=4)

    def handle_joint_moveit_stop_arm(self, _request):
        ...
        try:
            if self.arm_move_group:
                ...
                response.message = "arm move group stopped"
            else:
                response.success = False
                response.message = "arm move group is not initialized"
        except Exception as e:
            response.success = False
            response.message = f"failed to stop arm move group: {str(e)}"
        return response
